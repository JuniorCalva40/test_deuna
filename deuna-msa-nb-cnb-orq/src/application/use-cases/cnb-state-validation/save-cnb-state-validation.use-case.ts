...
@Injectable()
export class SaveCnbStateValidationUseCase {
  constructor(
    @Inject(CNB_STATE_VALIDATION_STORAGE_PORT)
    private readonly cnbStateStorage: CnbStateValidationStoragePort,
  ) {}
  async execute(id: string, data: CnbStateValidationDTO): Promise<void> {
    await this.cnbStateStorage.saveCnbStateValidation(id, data);
  }
}
