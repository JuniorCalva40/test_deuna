...
import { GetSignatureRequestUseCase } from './get-signature-request.use-case';
...
describe('GetSignatureRequestUseCase', () => {
  let useCase: GetSignatureRequestUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<GetSignatureRequestUseCase>(
      GetSignatureRequestUseCase,
    );
  });
  ...
  it('should retrieve a signature request successfully', async () => {
    ...
    jest
      .spyOn(storagePort, 'getSignatureRequest')
      .mockResolvedValue(mockSignatureRequestDto);

    // Act
    const result = await useCase.execute(identificationNumber);

    // Assert
    expect(storagePort.getSignatureRequest).toHaveBeenCalledWith(
      identificationNumber,
    );
    expect(result).toEqual(mockSignatureRequestDto);
  });

  it('should return null when signature request is not found', async () => {
    // Arrange
    const identificationNumber = '0987654321';
    jest.spyOn(storagePort, 'getSignatureRequest').mockResolvedValue(null);
  ...
  it('should handle errors properly', async () => {
    ...
    jest.spyOn(storagePort, 'getSignatureRequest').mockRejectedValue(testError);

    // Act & Assert
    await expect(useCase.execute(identificationNumber)).rejects.toThrow(
      testError,
    );
    expect(storagePort.getSignatureRequest).toHaveBeenCalledWith(
      identificationNumber,
