...
import { KycValidationConsumer } from './kyc-validation.consumer';
import { KycValidationService } from '@src/application/services/kyc-validation.service';
import { Logger } from '@nestjs/common';
import { KafkaContext } from '@nestjs/microservices';

describe('KycValidationConsumer', () => {
  let consumer: KycValidationConsumer;
  ...
  beforeEach(async () => {
    ...
    kafkaContextMock = {
      getTopic: jest.fn().mockReturnValue('cnb.kyc.queue'),
    ...
    consumer = module.get<KycValidationConsumer>(KycValidationConsumer);
  ...
  describe('onModuleInit', () => {
    it('should log initialization message', () => {
      ...
      expect(loggerMock.log).toHaveBeenCalledWith(
        'KycValidationConsumer initialized. Listening for messages in the cnb.kyc.queue topic',
      );
    });
  });
  ...
  describe('handleValidationRequest', () => {
    it('should process validation message successfully', async () => {
      ...
      // Act
      await consumer.handleValidationRequest(message, kafkaContextMock);
      ...
      expect(loggerMock.log).toHaveBeenCalledWith(
        'Topic: cnb.kyc.queue, Partition: 0, Offset: 100',
      );
      ...
      expect(loggerMock.log).toHaveBeenCalledWith(
        'Mensaje procesado exitosamente. Topic: cnb.kyc.queue, Partition: 0, Offset: 100',
      );
    });
    ...
    it('should handle and log errors, commit offset, and re-throw', async () => {
      ...
      // Act & Assert
      await expect(
        consumer.handleValidationRequest(message, kafkaContextMock),
    ...
    it('should handle invalid message format', async () => {
      ...
      // Act & Assert
      await expect(
        consumer.handleValidationRequest(invalidMessage, kafkaContextMock),
    ...
    it('should handle Kafka context errors', async () => {
      ...
      // Act & Assert
      await expect(
        consumer.handleValidationRequest(message, errorKafkaContextMock),
